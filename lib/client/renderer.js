  ...
  function prepareArc(treatment, radius) {
    ...
    if ( treatment.insulin > 0) {
      ...
      var enteredBy = '' + treatment.enteredBy;
       ...
       //tpaige: fix for autobolus display
      if ((treatment.insulin < 1.5 && !treatment.carbs ) || treatment.isSMB) { // don't show the unit of measurement for automatic boluses < 1 without carbs (e.g. oref0 SMB's). Otherwise lot's of small insulin only dosages are often unreadable
         unit_of_measurement = '';
  ...
  function appendLabels(treatmentDots, arc, opts) {
    ...
    if (opts.showLabels) {
      var label = treatmentDots.append('g')
        .attr('class', 'path')
        .attr('id', 'label')
        //tpaige: adjust orientation and size of labels
        .style('transform', 'rotate(-90deg) translateX(30px)')
        .style('font-size', '50%')
        .style('fill', 'white');
